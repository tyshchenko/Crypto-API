...
//
var find_richlist = (conn, coin) => new Promise((res, rej)=> {
  var Richlist_Model = Richlist(conn)
  Richlist_Model.findOne({coin: coin}, function(err, data) {
    if(data) {
      res(data);
    } else {
      rej(err);
...
module.exports = {
  ...
  check_stats: (conn, coin)=> new Promise((res, rej)=> {
    var Stats_Model = Stats(conn);
    Stats_Model.findOne({coin: coin}, function(err, stats) {
      if(stats) {
        res(true);
      } else {
        res(false);
  ...
  get_stats: (conn, coin) => new Promise((res, rej)=> {
    var Stats_Model = Stats(conn);
    Stats_Model.findOne({coin: coin}, function(err, stats) {
      if(stats) {
        res(stats._doc);
...
//
  create_stats: (conn, coin) => new Promise((res, rej)=> {
    var Stats_Model = Stats(conn);
    var newStats = new Stats_Model({
      coin: coin,
    });
...
//
  ...
  create_richlist: (conn, coin)=> new Promise((res, rej)=> {
      var Richlist_Model = Richlist(conn)
    var newRichlist = new Richlist_Model({
      coin: coin,
    });
  ...
  check_richlist: (conn, coin) => new Promise((res, rej)=> {
      var Richlist_Model = Richlist(conn)
    Richlist_Model.findOne({coin: coin}, function(err, exists) {
      if(exists) {
        res(true);
      } else {
        res(false);
  ...
  update_db: (conn, coin) => new Promise((res, rej)=>{
      var Stats_Model = Stats(conn);
      Promise.all([
          lib.get._blockcount(coin),
          lib.get_supply(coin,'TXOUTSET'),
