...
//
  ...
  update_richlist: (list, coin)=> new Promise((res, rej)=>{
    if(list == 'received') {
      Address.find({}).sort({received: 'desc'}).limit(100).exec(function(err, addresses){
        Richlist.update({coin: coin}, {
          received: addresses,
        }, function() {
          res('done');
        });
...
//
  ...
  update_tx_db: function(scoin, start, end, timeout, cb) {
    var coin = settings.coin.find(x => x.coin === scoin)
    var complete = false;
