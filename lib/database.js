var mongoose = require('mongoose')
  ...
  , Tx = require('../model/tx')
  , Richlist = require('../model/rich')
  , Peers = require('../model/peers')
...
//
module.exports = {
  createConnection: (coin) => new Promise((res, rej)=> {
      // var dbString = 'mongodb://localhost:27017/awardhuy'
  ...
  fn: (type, conn, dbmodel, params) => new Promise((res, rej) => {
      var model = require(`./model`)(conn);
      var newmodel = new model(params);
      switch (type) {
        case 'create':
          newmodel.save((err)=>{if(err){rej(err)} else {res('done')}});
          break;
        case 'check':
          console.log('here')
          model.findOne(params, (err, stats)=>{stats ? res(true): res(false)});
          break;
        case 'find':
          model.findOne(params, (err, stats)=>{stats ? res(stats): res(false)});
          break;
        case 'get':
          model.findOne({}, (err, stats)=>{stats ? res(stats): res([])});
          break;
      }
      // if (type == 'create'){
      //     newmodel.save((err)=>{if(err){rej(err)} else {res('done')}});
      // } else {
      //     model.findOne(params, (err, stats)=>{
      //         if(stats){
...
//
  ...
  // updates tx, address & richlist router's; called by sync.js
  update_tx_db: function(conn, coin, start, end, timeout, cb) {
    var Tx_Model = Tx(conn);
    var Stats_Model = Stats(conn);
    var scoin = require(`../initial/${coin}`);
    var complete = false;
